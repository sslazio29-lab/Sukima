...
- 現在の検証状態：`npm run check`（manifest 76ファイル自動生成）、`npm run audit` 成功。英語13単元すべて正答分布8/8/8（多義語のみ7/9/8、既存）。新規7単元は全て8/8/8。関係詞/時制・相/比較/句動詞は pages:check 200・count24 確認済み。
- 次にやること：push 後に助動詞/態・受動/準動詞も `npm.cmd run pages:check -- 英語 助動詞`（他2単元も同様）で配信確認。`npm run audit:candidates -- --subject 英語` で新単元の解説補強候補をチェック。英語の主要文法領域はほぼ網羅。さらに足すなら名詞・冠詞・代名詞、または会話表現・語彙系の細目。

## 2026-06-16  担当：Claude Code（その2：助動詞・態・準動詞）
- やったこと：取りこぼし単元をさらに3つ新設。助動詞（推量/義務、助動詞+have p.p. の後悔/推量、had better/may well 等）、態・受動（句動詞受動・知覚/使役の受動・SVOO受動・感情の前置詞・分詞修飾）、準動詞（to vs -ing、remember/forget/stop の意味変化、知覚/使役、形式主語、意味上の主語 for/of、分詞構文、完了動名詞）。各24問・分布8/8/8。
- 決めたこと／変更点：英語は13単元312問。新規ファイルのため既存進捗に影響なし。SPEC.md 未編集。
- つまずき・失敗・回避策：なし。check（manifest76）/audit pass。
- 次にやること：3単元の pages:check 配信確認、audit:candidates での解説点検。
- コミット：英語 助動詞・態・受動・準動詞 の3単元新設
