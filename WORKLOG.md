...
## 現在の引き継ぎ（毎回ここを最初に読む）
- 直近の主作業：英語の監査と拡充。(1) `英語_構文解釈.json` の和訳判断との重複4問（not so much as / Far from / remains to be seen / not because but because）を構造論点（分詞構文・関係副詞why・非制限which・it…that強調構文）へ差し替え、Q16の弱い誤答（挿入句/補語）を実候補（he / efficiency）へ修正。(2) 取りこぼし4単元を新設：`英語_関係詞``英語_時制・相``英語_比較``英語_句動詞・イディオム`（各24問）。英語は計10単元×24＝240問。
- 現在の検証状態：`npm run check`（manifest 73ファイル自動生成）、`npm run audit` 成功。英語10単元すべて正答分布ほぼ8/8/8（多義語のみ7/9/8、既存）。新規4単元は全て8/8/8。
- 次にやること：push 後に新4単元を `npm.cmd run pages:check -- 英語 関係詞`（時制・相 / 比較 / 句動詞・イディオム も同様）で配信確認。`npm run audit:candidates -- --subject 英語` で新単元の解説補強候補もチェック。さらに取りこぼすなら助動詞・態（受動）・準動詞（不定詞/動名詞/分詞）あたりが次の候補。
